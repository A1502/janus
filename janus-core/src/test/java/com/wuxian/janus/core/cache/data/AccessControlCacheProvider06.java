...
public class AccessControlCacheProvider06 extends BaseAccessControlCacheProvider {
    ...
    protected SourceLoader<IdType, OuterObjectTypeEntity> createOuterObjectTypeLoader() {
        SourceLoader<IdType, OuterObjectTypeEntity> result = () -> {
            ...
            map.put(IdBuilder.id(1), EntityBuilder.newOuterObjectTypeEntity(1, "outGroup1", "外部组1", "外部组1", true, false, 1, new Date(), 1, new Date()));
            map.put(IdBuilder.id(2), EntityBuilder.newOuterObjectTypeEntity(2, "outGroup2", "外部组2", "外部组2", true, false, 1, new Date(), 1, new Date()));
            return map;
        };
        return result;
    }
    ...
    protected SourceLoader<IdType, OuterObjectEntity> createOuterObjectLoader(IdType outerObjectTypeId) {
        SourceLoader<IdType, OuterObjectEntity> result = () -> {
            ...
            map.put(IdBuilder.id(1), EntityBuilder.newOuterObjectEntity(1, 1, "EMS001", "100", "销售", null, 1, new Date(), 1, new Date()));
            map.put(IdBuilder.id(2), EntityBuilder.newOuterObjectEntity(2, 2, "EMS002", "101", "HR", null, 1, new Date(), 1, new Date()));
            return map;
        };
        return result;
    }
...
//            map.put(IdBuilder.id(1), EntityBuilder.newUserOuterObjectXEntity(1, 1, 1, "1,2", 1, new Date(), 1, new Date()));
            map.put(IdBuilder.id(1), EntityBuilder.newUserOuterObjectXEntity(1, 1, null, 1, "1,2,3,", 1, new Date(), 1, new Date()));
//            map.put(IdBuilder.id(3), EntityBuilder.newUserOuterObjectXEntity(3, 1, 1, "", 1, new Date(), 1, new Date()));
            return map;
        };
        return result;
    }
