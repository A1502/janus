...
/*
    内置用户组ApplicationMaintainer and AGRoot--Single
    等号左边：OuterObject，UserGroup，Role
    等号右边：Permission，Role，user

*/
public class AccessControlCacheProvider14 extends BaseAccessControlCacheProvider {
    ...
    protected SourceLoader<IdType, ScopeUserGroupUserXStruct> createScopeUserGroupUserXLoader(ApplicationIdType applicationId, TenantIdType tenantId) {
        if (StrictUtils.equals(applicationId, IdBuilder.aId(defaultAId)) && StrictUtils.equals(tenantId, IdBuilder.tId(defaultTId))) {
            SourceLoader<IdType, ScopeUserGroupUserXStruct> result = () -> {
                ...
                map.put(IdBuilder.id(5), StructBuilder.newScopeUserGroupUserXStruct(5, null, 7, 1, 1, new Date(), 1, new Date()));
                return map;
            };
            return result;
        }
    ...
    protected SourceLoader<IdType, UserGroupStruct> createUserGroupLoader(ApplicationIdType applicationId, TenantIdType tenantId) {
        if (StrictUtils.equals(applicationId, IdBuilder.aId(defaultAId)) && StrictUtils.equals(tenantId, IdBuilder.tId(defaultTId))) {
            SourceLoader<IdType, UserGroupStruct> result = () -> {
                ...
                map.put(IdBuilder.id(2), StructBuilder.newUserGroupStruct(2, defaultAId, null, NativeUserGroupEnum.APPLICATION_ROOT.getCode(), NativeUserGroupEnum.APPLICATION_ROOT.getCode(), true, null, null, 1, new Date(), 1, new Date(), 1));
                map.put(IdBuilder.id(3), StructBuilder.newUserGroupStruct(3, defaultAId, null, NativeUserGroupEnum.TENANT_ROOT.getCode(), NativeUserGroupEnum.TENANT_ROOT.getCode(), true, null, null, 1, new Date(), 1, new Date(), 1));
    ...
    protected SourceLoader<IdType, UserGroupUserXStruct> createUserGroupUserXLoader(ApplicationIdType applicationId, TenantIdType tenantId) {
        if (StrictUtils.equals(applicationId, IdBuilder.aId(defaultAId)) && StrictUtils.equals(tenantId, IdBuilder.tId(defaultTId))) {
            SourceLoader<IdType, UserGroupUserXStruct> result = () -> {
                ...
                map.put(IdBuilder.id(5), StructBuilder.newUserGroupUserXStruct(5, 7, 1, true, true, false, false, false, false, false, false, false, false, 1, new Date(), 1, new Date(), 1));
                return map;
            };
            return result;
        }
    ...
    protected SourceLoader<IdType, RoleStruct> createSingleRoleLoader(ApplicationIdType applicationId, TenantIdType tenantId) {
        if (StrictUtils.equals(applicationId, IdBuilder.aId(defaultAId)) && StrictUtils.equals(tenantId, IdBuilder.tId(defaultTId))) {
            SourceLoader<IdType, RoleStruct> result = () -> {
                ...
                map.put(IdBuilder.id(3), StructBuilder.newRoleStruct(3, defaultAId, 10, NativeRoleEnum.TENANT_MAINTAINER.getCode(), false, NativeRoleEnum.TENANT_MAINTAINER.getCode(), true, null, null, null, NativeRoleEnum.TENANT_MAINTAINER.getCode(), 1, new Date(), 1, new Date(), 1));
                map.put(IdBuilder.id(4), StructBuilder.newRoleStruct(4, defaultAId, 10, NativeRoleEnum.TENANT_DATA_OWNER.getCode(), false, NativeRoleEnum.TENANT_DATA_OWNER.getCode(), true, null, null, null, NativeRoleEnum.TENANT_DATA_OWNER.getCode(), 1, new Date(), 1, new Date(), 1));
